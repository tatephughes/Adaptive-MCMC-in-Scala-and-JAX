...
for ((i = 1; i < 6; i++)); do
    sbt "runMain simple_run_IC ./data/scala_sample_basetest_IC_$label_$i" 
done

echo "Scala Outputs (MD)"
for ((i = 1; i < 6; i++)); do
    sbt "runMain simple_run_MD ./data/scala_sample_basetest_MD_$label_$i" 
done
...
echo "JAX Outputs (IC)"
for ((i = 1; i < 6; i++)); do
    sample_file="'./data/jax_sample_basetest_IC_${label}_${i}'"
...
echo "JAX Outputs (MD)"
for ((i = 1; i < 6; i++)); do
    sample_file="'./data/jax_sample_basetest_MD_${label}_${i}'"
