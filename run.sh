...
for ((i = 1; i < 6; i++)); do
    ...
    var_label="'${label}_${i}'" 
    Rscript -e "source('./src/main/R/AM_in_R.R'); sample <- main(sigma=read_sigma(100, './data/target_variance.csv'), n=10000, thinrate=100, burnin=0, write_files = TRUE, mix = FALSE, sample_file=${sample_file}, var_labels=${var_label}, seed=${i})"
done
...
for ((i = 1; i < 6; i++)); do
    ...
    var_label="'${label}_${i}'" 
    Rscript -e "source('./src/main/R/AM_in_R.R'); sample <- main(sigma=read_sigma(100, './data/target_variance.csv'), n=10000, thinrate=100, burnin=0, write_files = TRUE, mix = TRUE, sample_file=${sample_file}, var_labels=${var_label}, seed=${i})"
done
...
for ((i = 1; i < 6; i++)); do
    ...
    var_label="'${label}_${i}'" 
    ~/CPUJAX/bin/python -c "import sys; sys.path.append('./src/main/Python-JAX/'); from AM_in_JAX import *; main(sigma=read_sigma(100, './data/target_variance.csv'), n=10000, thinrate=100, burnin=0, write_files=True, mix=True, sample_file = ${sample_file}, use_64=False, var_labels=${var_label}, seed=${i})"
done
