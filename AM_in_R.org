...
compute_time_graph <- function(sigma, d){
  ...
  write.table(y, "./data/R_compute_times.csv", sep = ",", col.names = FALSE, row.names = FALSE)
...
main(d=10,n=100000, thinrate=10, burnin=10000, filepath = "./Figures/adaptive_trace_r_d_10.png")
...
#+begin_src R :session example :results graphics file :file ~/test_plot.png :height 600 :width 1400 :exports both

# The format of ~sample~ is actually kind of awkward, so we manually extract the relevant list of first coordinates
